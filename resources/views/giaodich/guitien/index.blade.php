...
@section('content')
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            @foreach ($guitien as $key => $gui)
                <tr>
                    <td class="text-center">{{ $key + 1 }}</td>
                    <td>{{ $gui->MaGDGuiTien }}</td>
                    <td class="text-center">{{ $gui->NgayTao }}</td>
                    <td class="text-center">{{ $gui->LoaiGiaoDich }}</td>


                    <td class="text-center">
                        <a class="btn btn-warning btn-edit-chitiet-giaodich-gui btn-edit-chitiet"
                            data-id="{{ $gui->MaGDGuiTien }}">
                            Chi tiết
                        </a>
                    </td>
                </tr>
